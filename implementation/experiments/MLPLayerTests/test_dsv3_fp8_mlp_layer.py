...
def test():
    ...
    chip = engine.SoftHier(softhier_root=args.softhier_root, kernel_root=args.kernel_root, output_root=args.output_root, tag="test_dsv3_fp8_mlp_layer")
    ...
    bs  = [1, 2, 4, 8, 16]
    sqs = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]
    for b in bs:
        for sq in sqs:
            # Default
            batch_size = b
            ...
            work.prefill_input_token = prefill_input_token
            info = {"llm": llm, "work": work}

            # Generate flow
            kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan = normal_llm.FFN_layer_plan(llm, work, arch)
            info['kernel_flow'] = kernel_flow
            info['spaceA_hbm_plan'] = spaceA_hbm_plan
            info['spaceB_hbm_plan'] = spaceB_hbm_plan
            normal_llm.hbm_plan_summary(spaceA_hbm_plan)
            normal_llm.hbm_plan_summary(spaceB_hbm_plan)
            print_dict_as_table(work.__dict__)

            # Dry Run
            flow.softhier_launch(chip, f"DRY-RUN b={batch_size} sq={prefill_input_token}", kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan, info=info, dry_run=True)

            # Full Run
            Results = flow.softhier_launch(chip, f"{llm.model_name} MLP b{batch_size} sq{prefill_input_token}", kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan, info=info)
