...
def decode_flatattn_plan(dtype, batch_size, speculative_length, kv_sequence_length, head_dimemsion, num_head, num_head_group, arch):
    #Basic Settings
    ...
    spaceA_hbm_addr                     = 0
    spaceB_hbm_addr                     = 0
    q_sequnece_length                   = speculative_length * num_head // num_head_group

    spaceA_hbm_plan["attn_q"] = {
        "addr"                          : spaceA_hbm_addr,
        "view"                          :(batch_size * num_head,  q_sequnece_length,  head_dimemsion),
        "shape"                         :(batch_size * q_sequnece_length * num_head,  head_dimemsion),
        "size"                          : batch_size * q_sequnece_length * num_head * head_dimemsion * elem_size,
        "tensor"                        : None
    }
    spaceA_hbm_addr                       += batch_size * q_sequnece_length * num_head * head_dimemsion * elem_size

    spaceB_hbm_plan["attn_k"] = {
        "addr"                          : spaceB_hbm_addr,
        "view"                          :(batch_size * num_head_group,  kv_sequence_length,  head_dimemsion),
        "shape"                         :(batch_size * kv_sequence_length * num_head_group,  head_dimemsion),
        "size"                          : batch_size * kv_sequence_length * num_head_group * head_dimemsion * elem_size,
        "tensor"                        : None
    }
    spaceB_hbm_addr                      += batch_size * kv_sequence_length * num_head_group * head_dimemsion * elem_size
    ...
    attn_mha.dtype                      = dtype
    attn_mha.kv_sequence_length         = kv_sequence_length
    attn_mha.q_sequence_length          = 1 # Decode
    attn_mha.speculative_length         = speculative_length
    attn_mha.head_dimemsion             = head_dimemsion
    attn_mha.num_head                   = num_head
    attn_mha.num_head_group             = num_head_group
    attn_mha.batch_size                 = batch_size
    attn_mha.flatten_numer              = 0
...
def test():
    ...
    chip = engine.SoftHier(softhier_root=args.softhier_root, kernel_root=args.kernel_root, output_root=args.output_root, tag="test_decode_flatmha")
    ...
    # Generate flow
    info = {}
    kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan = decode_flatattn_plan(dtype = tp, batch_size = b, speculative_length = sp, kv_sequence_length = kv, head_dimemsion = hd, num_head = nh, num_head_group = nhg, arch = arch)
    info['kernel_flow'] = kernel_flow
    info['spaceA_hbm_plan'] = spaceA_hbm_plan
    info['spaceB_hbm_plan'] = spaceB_hbm_plan

    # Dry Run
    flow.softhier_launch(chip, f"DRY-RUN b={b} sp={sp} kv={kv} hd={hd} nh={nh} nhg={nhg}", kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan, info=info, dry_run=True)

    # Full Run
    flow.softhier_launch(chip, f"FlatAttn decode b{b} sp{sp} kv{kv} hd{hd} nh{nh} nhg{nhg}", kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan, info=info)
    pass
