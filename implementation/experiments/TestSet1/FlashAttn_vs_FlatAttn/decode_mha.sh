...
for sp in "${sps[@]}"; do
    for kv in "${kvs[@]}"; do
        ...
        screen -dmS decode_mha_sp${sp}_kv${kv} bash -c "
            git clone git@github.com:gvsoc/gvsoc.git -b soft_hier_je decode_mha_sp${sp}_kv${kv}
            cd decode_mha_sp${sp}_kv${kv}
            ...
            sed -i \"246s/.*/    nhg = 32/\" \"implementation/${file_path}\"
            source sourceme.sh
            cd implementation
