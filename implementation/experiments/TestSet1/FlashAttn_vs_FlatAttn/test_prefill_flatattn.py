...
def prefill_flatattn_plan(dtype, batch_size, q_sequnece_length, head_dimemsion, num_head, num_head_group, arch):
    #Basic Settings
    ...
    spaceB_hbm_plan["attn_k"] = {
        ...
        "view"                          :(batch_size * num_head_group,  q_sequnece_length,  head_dimemsion),
        "shape"                         :(batch_size * q_sequnece_length * num_head_group,  head_dimemsion),
        "size"                          : batch_size * q_sequnece_length * num_head_group * head_dimemsion * elem_size,
        "tensor"                        : None
    }
    spaceB_hbm_addr                      += batch_size * q_sequnece_length * num_head_group * head_dimemsion * elem_size
    ...
    attn_mha.kv_sequence_length         = q_sequnece_length
    attn_mha.q_sequence_length          = q_sequnece_length
...
def test():
    ...
    chip = engine.SoftHier(softhier_root=args.softhier_root, kernel_root=args.kernel_root, output_root=args.output_root, tag="test_prefill_flatmha")
    ...
    # Generate flow
    info = {}
    kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan = prefill_flatattn_plan(dtype = tp, batch_size = b, q_sequnece_length = qs , head_dimemsion = hd, num_head = nh, num_head_group = nhg, arch = arch)
    info['kernel_flow'] = kernel_flow
    info['spaceA_hbm_plan'] = spaceA_hbm_plan
    info['spaceB_hbm_plan'] = spaceB_hbm_plan

    # Dry Run
    flow.softhier_launch(chip, f"DRY-RUN b={b} qs={qs} hd={hd} nh={nh} nhg={nhg}", kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan, info=info, dry_run=True)

    # Full Run
    flow.softhier_launch(chip, f"FlatAttn prefill b{b} qs{qs} hd{hd} nh{nh} nhg{nhg}", kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan, info=info)
    pass
