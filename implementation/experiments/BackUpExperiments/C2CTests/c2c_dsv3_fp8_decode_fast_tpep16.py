...
def test():
    ...
    # chip = engine.SoftHier(softhier_root=args.softhier_root, kernel_root=args.kernel_root, output_root=args.output_root, tag="c2c_dsv3_fp8_decode_fast_tpep16")
    c2c_engine = c2c.C2CEngine(softhier_root=args.softhier_root, output_root=args.output_root, tag="c2c_dsv3_fp8_decode_fast_tpep16")
    ...
    for kv in kvs:
        for sp in sps:
            ...
            c2c_engine.register_workload(f"dsv3_fp8_decode_bs{batch_size}_kv{kv_cache_length}_ep{expert_parallelsim}_sp{speculative_factor}", info)
