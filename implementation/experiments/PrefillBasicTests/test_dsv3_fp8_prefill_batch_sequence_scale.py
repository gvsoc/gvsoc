...
def test():
    ...
    chip = engine.SoftHier(softhier_root=args.softhier_root, kernel_root=args.kernel_root, output_root=args.output_root, tag="test_dsv3_fp8_prefill_batch_sequence_scale")
    ...
    for b in bs:
        for sq in sqs:
            ...
            # Dry Run
            flow.softhier_launch(chip, f"DRY-RUN b={batch_size} sq={prefill_input_token} ep={expert_parallelsim}", kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan, info=info, kernel_flow_simple=kernel_flow_simple, dry_run=True)

            # Full Run
            Results = flow.softhier_launch(chip, f"{llm.model_name} prefill b{batch_size} sq{prefill_input_token} ep{expert_parallelsim}", kernel_flow, spaceA_hbm_plan, spaceB_hbm_plan, info=info, kernel_flow_simple=kernel_flow_simple)
