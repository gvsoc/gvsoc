...
//
// SUMMA SpMM with sparse-aware DMA.
// W is stored in compact N:M format (PW_tile columns instead of N_tile).
// DMA loads/stores use PW_tile width, reducing memory traffic proportionally.

#ifndef _SUMMA_SPATZ_SPMM_H_
#define _SUMMA_SPATZ_SPMM_H_
...
#define PW_FROM_N(n) ((n) * SPMM_N_SPARSE / SPMM_M_SPARSE)
...
typedef struct SummaGEMMInfo
{
    ...
    uint32_t                    K_size;
    uint32_t                    M_tile;
    uint32_t                    N_tile;      // dense output tile width
    uint32_t                    PW_tile;     // compact weight tile width
    uint32_t                    K_tile;
    ...
    uint32_t                    L1_X1, L1_W1, L1_Z1;
    uint32_t                    L1_X2, L1_W2, L1_Z2;
    uint32_t                    L1_AREA;
    uint32_t                    L1_X_size, L1_W_size, L1_Z_size;
}SummaGEMMInfo;
...
{
    SummaGEMMInfo info;
    uint32_t PW_tile = PW_FROM_N(N_tile);
    uint32_t PW_size = PW_FROM_N(N_size);

    info.X_address = X_address;  info.W_address = W_address;  info.Z_address = Z_address;
    info.M_size = M_size;  info.N_size = N_size;  info.K_size = K_size;
    info.M_tile = M_tile;  info.N_tile = N_tile;  info.PW_tile = PW_tile;  info.K_tile = K_tile;
    info.group_reduction = group_reduction;  info.group_splitK = group_splitK;  info.group_splitN = group_splitN;
    info.summa_group_x = group_x;  info.summa_group_y = group_y;  info.summa_groups = num_group;
    ...
    info.store_recorded = 0;  info.store_m = 0;  info.store_n = 0;
    ...
    // Sparse-aware addressing: W uses PW_size stride, Z uses PW_size stride
    ...
    info.W_tile_base = W_address + W_address_group_gap * info.group.this_grid_id + info.cluster_in_group_id_x * PW_tile * DATA_TYPE_BYTE;
    info.Z_tile_base = Z_address + Z_address_group_gap * info.group.this_grid_id + info.cluster_in_group_id_y * M_tile * PW_size * DATA_TYPE_BYTE + info.cluster_in_group_id_x * PW_tile * DATA_TYPE_BYTE;
    ...
    info.W_tile_N_iter_offset = info.summa_group_x * PW_tile * DATA_TYPE_BYTE;
    info.W_tile_K_iter_offset = K_tile * PW_size * DATA_TYPE_BYTE;
    info.Z_tile_M_iter_offset = info.summa_group_y * M_tile * PW_size * DATA_TYPE_BYTE;
    info.Z_tile_N_iter_offset = info.summa_group_x * PW_tile * DATA_TYPE_BYTE;

    // L1 buffers use compact sizes
    info.L1_X1     = local(0);
    info.L1_W1     = info.L1_X1 + M_tile * K_tile * DATA_TYPE_BYTE;
    info.L1_Z1     = info.L1_W1 + PW_tile * K_tile * DATA_TYPE_BYTE;
    info.L1_X2     = info.L1_Z1 + M_tile * PW_tile * DATA_TYPE_BYTE;
    info.L1_W2     = info.L1_X2 + M_tile * K_tile * DATA_TYPE_BYTE;
    info.L1_Z2     = info.L1_W2 + PW_tile * K_tile * DATA_TYPE_BYTE;
    info.L1_AREA   = info.L1_Z2 + M_tile * PW_tile * DATA_TYPE_BYTE;
    info.L1_X_size = M_tile * K_tile * DATA_TYPE_BYTE;
    info.L1_W_size = PW_tile * K_tile * DATA_TYPE_BYTE;
    info.L1_Z_size = M_tile * PW_tile * DATA_TYPE_BYTE;
...
void SummaGEMMRun(SummaGEMMInfo * info)
{
    ...
    if (info->cluster_active)
    {
        ...
        for (int m = 0; m < info->M_iter; ++m)
        {
            ...
            {
                // Prefetch first tiles — W uses PW_tile width (sparse-aware DMA)
                if (flex_is_dm_core())
                {
                    if (info->cluster_for_rowwise == 1)
                    {
                        ...
                        {
                            flex_dma_async_broadcast(info->L1_X1, info->L1_X1, info->L1_X_size,
                    ...
                    {
                        // Sparse-aware: load PW_tile columns, stride PW_size
                        flex_dma_async_2d(
                            info->L1_W1,
                            info->W_tile_base + m * info->W_tile_M_iter_offset + n * info->W_tile_N_iter_offset,
                            info->PW_tile * DATA_TYPE_BYTE,
                            info->PW_tile * DATA_TYPE_BYTE,
                            PW_FROM_N(info->N_size) * DATA_TYPE_BYTE,
                            info->K_tile);
                        flex_dma_async_wait_all();
                        if (info->summa_group_y > 1)
                        {
                            flex_dma_async_broadcast(info->L1_W1, info->L1_W1, info->L1_W_size,
                ...
                for (int k = 1; k <= info->K_iter; ++k)
                {
                    ...
                    if (flex_is_dm_core())
                    {
                        if (k < info->K_iter)
                        {
                            ...
                            if (info->cluster_for_colwise == 1)
                            {
                                // Sparse-aware W prefetch
                                flex_dma_async_2d(DMA_L1_W,
                                    info->W_tile_base + m * info->W_tile_M_iter_offset + n * info->W_tile_N_iter_offset + k * info->W_tile_K_iter_offset,
                                    info->PW_tile * DATA_TYPE_BYTE, info->PW_tile * DATA_TYPE_BYTE,
                                    PW_FROM_N(info->N_size) * DATA_TYPE_BYTE, info->K_tile);
                                flex_dma_async_wait_all();
                                if (info->summa_group_y > 1)
                                {
                                    flex_dma_async_broadcast(DMA_L1_W, DMA_L1_W, info->L1_W_size,
                                        (ARCH_NUM_CLUSTER_X - 1), info->group.wakeup_col_mask);
                                    flex_dma_async_wait_all();
                                }
                            }
                        ...
                        // Store previous Z (sparse-aware: PW_tile width)
                        if (info->store_recorded == 1 && info->store_active == 1)
                        {
                                ...
                                ((info->cluster_in_group_id_x >= bid || info->cluster_in_group_id_x < eid) && eid <= bid))
                            {
                                ...
                                {
                                    flex_dma_async_reduction(DMA_L1_Z, DMA_L1_Z, info->L1_Z_size,
                                        COLLECTIVE_REDSUM_TYPE, ~info->group.wakeup_row_mask, ~info->group.wakeup_col_mask);
                                    flex_dma_async_wait_all();
                                }
                                flex_dma_async_2d(
                ...
                if (info->group_reduction == 1) { flex_global_barrier_xy(); }
                else { grid_sync_group_barrier_xy(&(info->group)); }
                info->store_recorded = 1;  info->store_m = m;  info->store_n = n;
                info->store_id = info->summa_group_x;
                uint32_t tmp_z = DMA_L1_Z;  DMA_L1_Z = COMPUTE_L1_Z;  COMPUTE_L1_Z = tmp_z;
                flex_intra_cluster_sync();
            }
        }

        // Final store (sparse-aware)
