...
int main()
{
    ...
    if (ACTI_ACTI_NUMER)
    {
        ...
        if (flex_get_cluster_id() == 0 && flex_is_dm_core())
        {
            ...
            uint64_t output_size = ACTI_M_SIZE * ACTI_N_SIZE * DATA_TYPE_BYTE;
            uint64_t num_output_chunk = (output_size + ACTI_ACTI_NUMER_CHUNK - 1) / ACTI_ACTI_NUMER_CHUNK;

            //2. iterate over chunks
            flex_dump_open();
            for (uint64_t i = 0; i < num_output_chunk; ++i)
            {
                flex_dump_hbm(i * ACTI_ACTI_NUMER_CHUNK + O_EADDR - ARCH_HBM_START_BASE, ACTI_ACTI_NUMER_CHUNK);
                flex_dump_hbm(i * ACTI_ACTI_NUMER_CHUNK + O_GADDR - ARCH_HBM_START_BASE, ACTI_ACTI_NUMER_CHUNK);
            }
            flex_dump_close();
        }
    }
