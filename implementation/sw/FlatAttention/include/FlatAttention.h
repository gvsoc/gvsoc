...
// #define DEBUG
void log_out_l1_info(FlatAttentionInfo * info, uint32_t i){
    grid_sync_group_barrier_xy(&(info->group));
    if (flex_is_dm_core())
    {
        flex_dump_set_base(i*16 + 0);
        flex_dma_async_1d((ARCH_CLUSTER_TCDM_BASE+ARCH_CLUSTER_TCDM_SIZE), info->L1_Q, info->L1_Q_size);
        flex_dma_async_wait_all();

        flex_dump_set_base(i*16 + 1);
        flex_dma_async_1d((ARCH_CLUSTER_TCDM_BASE+ARCH_CLUSTER_TCDM_SIZE), info->L1_KT, info->L1_KT_size);
        flex_dma_async_wait_all();

        flex_dump_set_base(i*16 + 2);
        flex_dma_async_1d((ARCH_CLUSTER_TCDM_BASE+ARCH_CLUSTER_TCDM_SIZE), info->L1_V, info->L1_KT_size);
        flex_dma_async_wait_all();

        flex_dump_set_base(i*16 + 3);
        flex_dma_async_1d((ARCH_CLUSTER_TCDM_BASE+ARCH_CLUSTER_TCDM_SIZE), info->L1_O, info->L1_O_size);
        flex_dma_async_wait_all();

        flex_dump_set_base(i*16 + 4);
        flex_dma_async_1d((ARCH_CLUSTER_TCDM_BASE+ARCH_CLUSTER_TCDM_SIZE), info->L1_A, info->L1_A_size);
        flex_dma_async_wait_all();

        flex_dump_set_base(i*16 + 5);
        flex_dma_async_1d((ARCH_CLUSTER_TCDM_BASE+ARCH_CLUSTER_TCDM_SIZE), info->L1_m, info->L1_m_size);
        flex_dma_async_wait_all();

        flex_dump_set_base(i*16 + 6);
        flex_dma_async_1d((ARCH_CLUSTER_TCDM_BASE+ARCH_CLUSTER_TCDM_SIZE), info->L1_l, info->L1_l_size);
        flex_dma_async_wait_all();

        flex_dump_set_base(i*16 + 7);
        flex_dma_async_1d((ARCH_CLUSTER_TCDM_BASE+ARCH_CLUSTER_TCDM_SIZE), info->L1_e, info->L1_e_size);
        flex_dma_async_wait_all();
    }
    grid_sync_group_barrier_xy(&(info->group));
}
...
int flat_attention(
...
#ifdef DEBUG
    if (flex_is_dm_core()) flex_dump_open();
#endif
    flex_global_barrier_xy();
    ...
    flex_global_barrier_xy();
#ifdef DEBUG
    if (flex_is_dm_core()) flex_dump_close();
#endif
    ...
    if (dump_enable)
    {
        ...
        {
            flex_dump_open();
            flex_dump_hbm(O_base_address - ARCH_HBM_START_BASE, 1024);
            flex_dump_hbm(O_golden_base_address - ARCH_HBM_START_BASE, 1024);
            flex_dump_close();
        }
    }
    flex_global_barrier_xy();
...
//                     flex_dma_async_1d(
//                         info->DB_L1_KT,
//                         info->HBM_K + (iter_j + (2*iter_h + 1)*info->Tc) * info->slice_KTV_size,
//                         info->L1_KT_size);
//                     flex_dma_async_wait_all();
//                 }
...
//                     flex_dma_async_1d(
//                         info->L1_KT,
//                         info->HBM_K + (iter_j_ + 2*iter_h_*info->Tc) * info->slice_KTV_size,
//                         info->L1_KT_size);
//                     flex_dma_async_wait_all();
//                 }
