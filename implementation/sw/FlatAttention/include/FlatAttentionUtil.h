...
FlatAttentionInfo flat_attention_analyze(
    ...
    uint32_t flatten_slice_x    = flatten_scale_x;
    uint32_t flatten_slice_y    = flatten_scale_y;
    ...
    //L1 location information
    info.L1_Q                   = local(0);
    info.L1_KT                  = info.L1_Q     + info.Br_s * info.d    * DATA_TYPE_BYTE;
    info.L1_V                   = info.L1_KT    + info.d    * info.Bc_s * DATA_TYPE_BYTE;
    info.L1_O                   = info.L1_V     + info.Bc_s * info.d    * DATA_TYPE_BYTE;
    info.L1_A                   = info.L1_O     + info.Br_s * info.d    * DATA_TYPE_BYTE;
    info.DB_L1_Q                = info.L1_A     + info.Br_s * info.Bc_s * DATA_TYPE_BYTE;
    info.DB_L1_KT               = info.DB_L1_Q  + info.Br_s * info.d    * DATA_TYPE_BYTE;
    info.DB_L1_V                = info.DB_L1_KT + info.d    * info.Bc_s * DATA_TYPE_BYTE;
    info.DB_L1_O                = info.DB_L1_V  + info.Bc_s * info.d    * DATA_TYPE_BYTE;
    info.DB_L1_A                = info.DB_L1_O  + info.Br_s * info.d    * DATA_TYPE_BYTE;
    info.L1_m                   = info.DB_L1_A  + info.Br_s * info.Bc_s * DATA_TYPE_BYTE;
    ...
    info.DB_L1_m                = info.L1_lr    + info.Br_s * 1         * DATA_TYPE_BYTE;
    info.DB_L1_l                = info.DB_L1_m  + info.Br_s * 1         * DATA_TYPE_BYTE;
    info.DB_L1_e                = info.DB_L1_l  + info.Br_s * 1         * DATA_TYPE_BYTE;
    info.DB_L1_mr               = info.DB_L1_e  + info.Br_s * 1         * DATA_TYPE_BYTE;
    info.DB_L1_lr               = info.DB_L1_mr + info.Br_s * 1         * DATA_TYPE_BYTE;

    //Usefull parameters for data layout address calculation
    info.L1_Q_size              = info.Br_s     * info.d    * DATA_TYPE_BYTE;
    info.L1_KT_size             = info.d        * info.Bc_s * DATA_TYPE_BYTE;
    ...
    info.L1_e_size              = info.Br_s     * 1         * DATA_TYPE_BYTE;
...
inline void flat_attention_vector_rowmax_MV_V(uint32_t num_cols, uint32_t num_rows, uint32_t M_src, uint32_t V_src, uint32_t V_dst, attn_data_t sqrt_dim){
    uint32_t vl;
