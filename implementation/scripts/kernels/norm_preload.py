...
def gen_rmsnorm_preload_numpy_arrays(norm):
    ...
    fptype = torch.float8_e5m2 if norm.dtype == "fp8" else torch.float16
    ...
    # RMSNorm
    I_fptype = rand_fptype((norm.m_size,  norm.n_size), device)
    I = I_fptype.to(compute_dtype)
    rms = I.pow(2).mean(dim=1, keepdim=True).sqrt()
    O = I / rms
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate C header files from a RMSNorm preload file and Dynamically import Python modules from absolute or relative paths.")
    parser.add_argument("elf_path", type=str, help="Path of preload elf file")
    parser.add_argument("header_path", type=str, help="Path to RMSNorm preload C header file")
    parser.add_argument(
        "module_paths",
        metavar="module_path",
        type=str,
    ...
    I_np, O_empty, O_golden = gen_rmsnorm_preload_numpy_arrays(norm)
