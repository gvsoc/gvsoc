#
# Copyright (C) 2025 ETH Zurich and University of Bologna
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#

# Author: Chi Zhang <chizhang@ethz.ch>

import re
import os
import io
import sys
import math
import shutil
import torch
import argparse
import numpy as np
import importlib.util
from tqdm import tqdm
import preload as pld

def import_module_from_path(module_path):
    """
    Dynamically import a module from an absolute path and mimic `from module import *`.
    """
    module_name = os.path.splitext(os.path.basename(module_path))[0]  # Extract the file name without extension
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None:
        raise ImportError(f"Cannot find a module at path: {module_path}")
    
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    
    # Mimic `from module import *`
    globals().update(vars(module))
    return module

def gen_attention_preload_numpy_arrays(b, d_nope, d_rope, sq, sc, dt):
    # ---- params you can change ----
    seed = 0
    # --------------------------------

    torch.manual_seed(seed)

    # pick device; fp8 compute is limited, so we do compute in f16/f32 and store in fp8
    device = torch.device("cpu")

    # make sure FP8 (e5m2) is available in this PyTorch
    if not hasattr(torch, "float8_e5m2"):
        raise RuntimeError("This PyTorch build does not expose torch.float8_e5m2.")

    fptype = torch.float8_e5m2 if dt == "fp8" else torch.float16

    # helper: create random fptype tensors by sampling in f16 then casting to fptype for storage
    def rand_fptype(shape, device):
        # sample in a safe range to reduce saturation when casting to fptype
        # normal(0, 0.5) tends to survive e5m2 rounding reasonably well
        x = torch.randn(shape, device=device, dtype=torch.float32) * 0.1 + 0.0625
        return x.to(fptype)

    # per-head Q,K,V in fptype
    QNs = []
    QRs = []
    CNs = []
    CRs = []
    Os = []

    for _ in range(b):
        QN_fptype = rand_fptype((sq,  d_nope), device)
        QR_fptype = rand_fptype((sq,  d_rope), device)
        CN_fptype = rand_fptype((sc,  d_nope), device)
        CR_fptype = rand_fptype((sc,  d_rope), device)

        # compute attention in higher precision, then (optionally) store O in fptype
        # cast up for matmuls/softmax stability
        # - on CUDA, float16 or bfloat16 is fine; float32 is safest everywhere.
        compute_dtype = torch.float16

        QN = QN_fptype.to(compute_dtype)
        QR = QR_fptype.to(compute_dtype)
        CN = CN_fptype.to(compute_dtype)
        CR = CR_fptype.to(compute_dtype)

        attn_scores_nope = QN @ CN.transpose(0, 1)
        attn_scores_rope = QR @ CR.transpose(0, 1)
        attn_scores = (attn_scores_nope + attn_scores_rope) / math.sqrt(d_nope + d_rope)     # [sq, sc]
        attn_probs  = torch.softmax(attn_scores, dim=-1)         # [sq, sc]
        O = attn_probs @ CN                                       # [sq, d_nope]

        # store results: keep Q,K,V in fptype as requested; also provide O in fptype
        O_fptype = O.to(fptype)

        QNs.append(QN_fptype)
        QRs.append(QR_fptype)
        CNs.append(CN_fptype)
        CRs.append(CR_fptype)
        Os.append(O_fptype)

    # concatenate heads:
    # Q, O -> [sq, h*d]; K, V -> [skv, h*d]
    QN_cat_fptype = torch.cat(QNs, dim=0)
    QR_cat_fptype = torch.cat(QRs, dim=0)
    CN_cat_fptype = torch.cat(CNs, dim=0)
    CR_cat_fptype = torch.cat(CRs, dim=0)
    O_cat_fptype  = torch.cat(Os, dim=0)

    # convert to NumPy: cast to float16 (NumPy may not support PyTorch fptype dtype)
    if fptype == torch.float8_e5m2:
        QN_np = QN_cat_fptype.view(torch.uint8).cpu().numpy()
        QR_np = QR_cat_fptype.view(torch.uint8).cpu().numpy()
        CN_np = CN_cat_fptype.view(torch.uint8).cpu().numpy()
        CR_np = CR_cat_fptype.view(torch.uint8).cpu().numpy()
        O_np  = O_cat_fptype.view(torch.uint8).cpu().numpy()
    else:
        QN_np = QN_cat_fptype.view(torch.uint16).cpu().numpy()
        QR_np = QR_cat_fptype.view(torch.uint16).cpu().numpy()
        CN_np = CN_cat_fptype.view(torch.uint16).cpu().numpy()
        CR_np = CR_cat_fptype.view(torch.uint16).cpu().numpy()
        O_np  = O_cat_fptype.view(torch.uint16).cpu().numpy()

    # quick sanity prints
    print("QN_cat_fptype:", tuple(QN_cat_fptype.shape), QN_cat_fptype.dtype)
    print("QR_cat_fptype:", tuple(QR_cat_fptype.shape), QR_cat_fptype.dtype)
    print("CN_cat_fptype:", tuple(CN_cat_fptype.shape), CN_cat_fptype.dtype)
    print("CR_cat_fptype:", tuple(CR_cat_fptype.shape), CR_cat_fptype.dtype)
    print("O_cat_fptype:", tuple(O_cat_fptype.shape), O_cat_fptype.dtype)

    print("QN_np:", QN_np.shape, QN_np.dtype)
    print("QR_np:", QR_np.shape, QR_np.dtype)
    print("CN_np:", CN_np.shape, CN_np.dtype)
    print("CR_np:", CR_np.shape, CR_np.dtype)
    print("O_np:", O_np.shape, O_np.dtype)

    O_empty = np.zeros(O_np.shape, dtype=O_np.dtype)
    O_golden = O_np

    return QN_np, QR_np, CN_np, CR_np, O_empty, O_golden


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate C header files from a FlatMLA preload file and Dynamically import Python modules from absolute or relative paths.")
    parser.add_argument("elf_path", type=str, help="Path of preload elf file")
    parser.add_argument("header_path", type=str, help="Path to FlatMLA preload C header file")
    parser.add_argument(
        "module_paths",
        metavar="module_path",
        type=str,
        nargs="+",
        help="Paths to Python modules to import (absolute or relative)."
    )

    args = parser.parse_args()

    # Process each provided module path
    for module_path in args.module_paths:
        # Convert to absolute path
        absolute_path = os.path.abspath(module_path)

        if not os.path.isfile(absolute_path):
            print(f"Error: {absolute_path} is not a valid file.")
            continue

        try:
            # Import the module dynamically
            module = import_module_from_path(absolute_path)
            print(f"Successfully imported: {module.__name__} from {absolute_path}")
        except Exception as e:
            print(f"Failed to import {absolute_path}: {e}")

    #instanciate arch and attn configurations
    attn = FlatMLA()
    arch = FlexClusterArch()

    #generate numpy array
    QN_np, QR_np, CN_np, CR_np, O_empty, O_golden = gen_attention_preload_numpy_arrays(attn.batch_size, attn.nope_head_dim, attn.rope_head_dim, attn.q_sequence_length * attn.speculative_length * attn.num_head, attn.kv_sequence_length, attn.dtype)
    QN_addr = arch.hbm_start_base
    QR_addr = QN_addr + QN_np.nbytes
    O_eaddr = QR_addr + QR_np.nbytes
    O_gaddr = O_eaddr + O_empty.nbytes
    CN_addr = arch.hbm_start_base + arch.hbm_node_addr_space * 2 * arch.num_cluster_y + arch.hbm_node_addr_space * arch.num_cluster_x
    CR_addr = CN_addr + CN_np.nbytes
    print(f"QN_addr = {QN_addr: #x}")
    print(f"QR_addr = {QR_addr: #x}")
    print(f"O_eaddr = {O_eaddr: #x}")
    print(f"O_gaddr = {O_gaddr: #x}")
    print(f"CN_addr = {CN_addr: #x}")
    print(f"CR_addr = {CR_addr: #x}")

    #generate preload elf
    if attn.flatten_numer == 1:
        pld.make_preload_elf(args.elf_path, 
            [QN_np,    QR_np,    CN_np,    CR_np,    O_empty,  O_golden],
            [QN_addr,  QR_addr,  CN_addr,  CR_addr,  O_eaddr,  O_gaddr])
    else:
        dumnp = np.array([1,1,1,1,1])
        pld.make_preload_elf(args.elf_path, 
            [dumnp,    dumnp,    dumnp,    dumnp,    dumnp,    dumnp],
            [QN_addr,  QR_addr,  CN_addr,  CR_addr,  O_eaddr,  O_gaddr])
        pass

    #generate preload header file
    with open(args.header_path, 'w') as file:
        file.write('#ifndef _TMLA_PRELOAD_H_\n')
        file.write('#define _TMLA_PRELOAD_H_\n\n')
        file.write(f'#define {"QN_addr".upper()} ((uint64_t){QN_addr: #x})\n')
        file.write(f'#define {"QR_addr".upper()} ((uint64_t){QR_addr: #x})\n')
        file.write(f'#define {"O_eaddr".upper()} ((uint64_t){O_eaddr: #x})\n')
        file.write(f'#define {"O_gaddr".upper()} ((uint64_t){O_gaddr: #x})\n')
        file.write(f'#define {"CN_addr".upper()} ((uint64_t){CN_addr: #x})\n')
        file.write(f'#define {"CR_addr".upper()} ((uint64_t){CR_addr: #x})\n')
        file.write('\n#endif // _TMLA_PRELOAD_H_\n')

    print(f'Header file "{args.header_path}" generated successfully.')

