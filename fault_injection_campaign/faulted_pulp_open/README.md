...
- Instantiate `fault_injection.fic` in `pulp_open/soc.py` (`fic = FIC(self, "fic")`)
- Connect the above `FIC` to `axi_ico` in `pulp_open/soc.py` (`fic.o_GLOBAL_AS(axi_ico.i_INPUT())`) to enable access into global address space
