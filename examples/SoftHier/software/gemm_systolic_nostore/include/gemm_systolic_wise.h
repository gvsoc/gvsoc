...
GemmSystolicInfo gemm_systolic_wise_analysis(
    ...
    info.total_iter = ((K_tile + ARCH_NUM_CLUSTER_X -1)/ARCH_NUM_CLUSTER_X) * ((M_tile + ARCH_NUM_CLUSTER_Y - 1)/ARCH_NUM_CLUSTER_Y) * N_tile + ARCH_NUM_CLUSTER_X + ARCH_NUM_CLUSTER_Y;
...
void gemm_systolic_wise_compute_dma_access(GemmSystolicInfo * info, uint32_t iter){
    ...
    if ((iter >= info->systolic_delay) && (iter < (info->Z_tile_all * info->XW_tile_length + info->systolic_delay)))
    {
        uint32_t eff_iter = iter - info->systolic_delay;
        uint32_t xw_count = eff_iter;
        FlexPosition pos = get_pos(flex_get_cluster_id());
...
void gemm_systolic_wise_compute_redmule_action(GemmSystolicInfo * info, uint32_t iter){
    ...
    if ((iter >= (info->systolic_delay + 1)) && (iter < (info->Z_tile_all * info->XW_tile_length + info->systolic_delay + 1)))
    {
