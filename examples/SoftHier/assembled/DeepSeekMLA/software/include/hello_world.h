...
void test_tile(){
    ...
    if (flex_is_dm_core() && flex_get_cluster_id() == 0)
    {
        ...
        //redmule
        flex_redmule_config(128, 64, 64);
        flex_redmule_trigger((uint32_t)x, (uint32_t)w, (uint32_t)z, REDMULE_UINT_16);
        flex_redmule_wait();

        flex_redmule_config(128, 128, 64);
        flex_redmule_trigger((uint32_t)x, (uint32_t)w, (uint32_t)z, REDMULE_UINT_16);
        flex_redmule_wait();

        flex_redmule_config(128, 512, 64);
        flex_redmule_trigger((uint32_t)x, (uint32_t)w, (uint32_t)z, REDMULE_UINT_16);
        flex_redmule_wait();

        flex_redmule_config(128, 64, 192);
        flex_redmule_trigger((uint32_t)x, (uint32_t)w, (uint32_t)z, REDMULE_UINT_16);
        flex_redmule_wait();
        ...
        flex_redmule_config(128, 192, 128);
        flex_redmule_trigger((uint32_t)x, (uint32_t)w, (uint32_t)z, REDMULE_UINT_16);
        flex_redmule_wait();

        flex_redmule_config(128, 132, 128);
        flex_redmule_trigger((uint32_t)x, (uint32_t)w, (uint32_t)z, REDMULE_UINT_16);
        flex_redmule_wait();
    }
    flex_global_barrier_xy();//Global barrier
...
void test_hbm_alias(){
    ...
    if (flex_is_dm_core() && pos.x == 0)
    {
        ...
        for (int i = 0; i < 32; ++i)
        {
            bare_dma_start_2d(local(0),hbm_addr(0) + pos.y * 128, 128, 128, 128*ARCH_NUM_CLUSTER_Y, 512); //Start iDMA
...
void test_basic_task_one_tile(){
    ...
    if (pos.x == 0 && pos.y == 0)
    {
        if (flex_is_dm_core())
        {
            /* DMA1 for transaction on west HBM*/
            flex_dma_async_1d(local(0),hbm_west(0,0), M*K*2);
            flex_dma_async_1d(local(0),hbm_south(0,0), N*K*2);
            flex_dma_async_wait_all();
        }

        flex_intra_cluster_sync();//Intra Cluster barrier

        if (flex_is_first_core())
        {
            /* Core0 for task scheduling and result collecting */
