...
void DeepSeek_SUMMA_2D_Weight_Stationary(
    uint64_t                    X_address,
    uint64_t                    W_address,
    uint64_t                    Z_address,
    uint32_t                    M_size,
    ...
    if (info.cluster_active)
    {
            ...
            for (int n = 0; n < info.N_iter; ++n)
            {
                ...
                if (flex_is_dm_core())
                {
                    ...
                    if (info.cluster_in_group_id_y == 0)
                    {
                        ...
                        flex_dma_async_1d(
                            info.L1_W1, /*destination*/
                            info.W_tile_base, /*source*/
                            info.K_tile * info.N_tile * info.elem_size /*transfer size*/); //Start 1D iDMA
                        flex_dma_async_wait_all(); // Wait for iDMA Finishing
                        //col-wise multicast
                        if (info.summa_group_y > 1)
                        {
        ...
        {
            flex_global_barrier_xy();
            if (flex_is_dm_core() && info.group.this_grid_id == 0)
            {
                //Reduce Z
                flex_dma_async_reduction(
                    info.L1_Z2/*dst_offset*/,
                    info.L1_Z2/*src_offset*/,
