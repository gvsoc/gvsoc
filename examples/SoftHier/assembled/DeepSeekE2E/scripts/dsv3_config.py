...
parser = argparse.ArgumentParser(description="Generate C header files from a DeepSeekV3 configuration file.")
parser.add_argument("input_file",  nargs="?", help="Path to DeepSeekV3 configuration python file")
parser.add_argument("output_file", nargs="?", help="Path to DeepSeekV3 configuration C header file")
...
with open(C_header_file, 'w') as file:
    file.write('#ifndef _DEEPSEEKV3_CONFIG_H_\n')
    file.write('#define _DEEPSEEKV3_CONFIG_H_\n\n')
    
    for attr_name, attr_value in attributes.items():
        # Convert attribute name to uppercase and prefix with 'ARCH_'
        define_name = f'DSV3_{attr_name.upper()}'
        if define_name == 'DSV3_DTYPE':
            if attr_value == "'fp16'":
                ...
                file.write(f'typedef uint16_t                    deepseek_data_t;\n')
                ...
                file.write(f'typedef uint8_t                     deepseek_data_t;\n')
                file.write(f'#define COLLECTIVE_REDSUM_TYPE      COLLECTIVE_REDADD_FP_16\n')
                file.write(f'#define COLLECTIVE_REDMAX_TYPE      COLLECTIVE_REDMAX_FP_16\n')
            continue
