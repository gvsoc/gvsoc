...
class FlexClusterDoL1dsSystem(gvsoc.systree.Component):    # DoL: DRAM on Logic
    ...
    def __init__(self, parent, name, parser):
        ...
        # @NOTE: for DoL, right now one ctrl per 
        ...
        if not hasattr(arch, 'hbm_type'): arch.hbm_type = 'hbm2-16ch'
        ...
        #     pass
        # @NOTE: for DoL, only one DRAMsys instance
        hbm_channels = memory.dramsys.Dramsys(self, 'hbm_channels', dram_type=arch.hbm_type)    # @TODO: change standard definition to match how it's being integrated
        ...
        #     pass
        # @NOTE: for DoL, one ctrl for each node, and directly connect to DRAMsys without router in between. Also adding one interface router per cluster.
        hbm_ctrl_list = []
        hbm_router_list = []
        for node_id in range(num_hbm_ctrl):
            ...
            hbm_router_list[node_id].add_mapping('output', add_offset=node_id * arch.hbm_node_addr_space)
            self.bind(hbm_router_list[node_id], 'output', hbm_ctrl_list[node_id], f'in_0')  # Bind the router output to the  hbm_ctrl
            self.bind(hbm_ctrl_list[node_id], 'out_0', hbm_channels, f'input')      # Bind the hbm_ctrl output to the hbm channel
            pass
        ...
        #Data NoC + Sync NoC    @NOTE: map o_WIDE_SOC to local hbm interface and to NoC, and map the NoC to cluster remote TCDM and local hbm interface
        for node_id in range(num_clusters):
            ...
            cluster_list[node_id].o_WIDE_SOC(cluster_out_router_list[node_id].i_INPUT())    # Connect to the output router
            cluster_out_router_list[node_id].o_MAP(data_noc.i_CLUSTER_INPUT(x_id, y_id))    # By default, route to wide NoC
...
class FlexClusterDoL1dsBoard(gvsoc.systree.Component):

    def __init__(self, parent, name, parser, options):
        super(FlexClusterDoL1dsBoard, self).__init__(parent, name, options=options)

        arch  = FlexClusterArch()
        clock = Clock_domain(self, 'clock', frequency=(1000000000 if not hasattr(arch, 'frequence') else arch.frequence))

        flex_cluster_system = FlexClusterDoL1dsSystem(self, 'chip', parser)
...
class Target(gvsoc.runner.Target):
    ...
    def __init__(self, parser, options):
        super(Target, self).__init__(parser, options,
            model=FlexClusterDoL1dsBoard, description="Flex Cluster virtual board")