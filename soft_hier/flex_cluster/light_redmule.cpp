...
fp8e5m2 fp8e5m2_fma(fp8e5m2 a, fp8e5m2 b, fp8e5m2 c) {
    float fa = fp8e5m2_to_float(a);
    float fb = fp8e5m2_to_float(b);
    float fc = fp8e5m2_to_float(c);

    float result = fa * fb + fc;
    return float_to_fp8e5m2(result);
}
...
void matmul_fp8e5m2(fp8e5m2 *z, fp8e5m2 *y, fp8e5m2 *x, fp8e5m2 *w, uint16_t m_size, uint16_t n_size, uint16_t k_size) {
    for (int i = 0; i < m_size; ++i) {
        for (int j = 0; j < k_size; ++j) {
            z[i * k_size + j] = y[i * k_size + j];
            for (int k = 0; k < n_size; ++k) {
                z[i * k_size + j] = fp8e5m2_fma(x[i * n_size + k], w[k * k_size + j], z[i * k_size + j]);
            }
        }
    }
}