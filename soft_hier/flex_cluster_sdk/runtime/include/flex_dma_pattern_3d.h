...
inline void bare_dma_set_mask(uint8_t row_mask, uint8_t col_mask, uint8_t lay_mask){
    uint32_t mask = col_mask;
    mask = (mask << 8) | row_mask;
    mask = (mask << 8) | lay_mask;
...
inline uint32_t bare_dma_start_1d_broadcast(uint64_t dst, uint64_t src,
                            size_t size, uint8_t row_mask, uint8_t col_mask, uint8_t lay_mask) {
    register uint32_t reg_dst_low asm("a0") = dst >> 0;    // 10
    register uint32_t reg_dst_high asm("a1") = dst >> 32;  // 11
    register uint32_t reg_src_low asm("a2") = src >> 0;    // 12
    register uint32_t reg_src_high asm("a3") = src >> 32;  // 13
    ...
    bare_dma_set_mask(row_mask, col_mask, lay_mask);
...
inline uint32_t bare_dma_start_1d_reduction(uint64_t dst, uint64_t src,
        size_t size, collective_compute_format_t fmt, uint8_t row_mask, uint8_t col_mask, uint8_t lay_mask) {
    register uint32_t reg_dst_low asm("a0") = dst >> 0;    // 10
    register uint32_t reg_dst_high asm("a1") = dst >> 32;  // 11
    register uint32_t reg_src_low asm("a2") = src >> 0;    // 12
    register uint32_t reg_src_high asm("a3") = src >> 32;  // 13
...
void flex_dma_async_broadcast(uint64_t dst_offset, uint64_t src_offset, size_t transfer_size, uint8_t row_mask, uint8_t col_mask, uint8_t lay_mask){
    FlexPosition pos = get_pos(flex_get_cluster_id());
    bare_dma_start_1d_broadcast(remote_pos(pos,dst_offset), local(src_offset), transfer_size, row_mask, col_mask, lay_mask); //Start iDMA
}

void flex_dma_async_reduction(uint64_t dst_offset, uint64_t src_offset, size_t transfer_size, collective_compute_format_t fmt, uint8_t row_mask, uint8_t col_mask, uint8_t lay_mask){
    FlexPosition pos = get_pos(flex_get_cluster_id());
    bare_dma_start_1d_reduction(local(dst_offset), remote_pos(pos,src_offset), transfer_size, fmt, row_mask, col_mask, lay_mask); //Start iDMA
}
...
void flex_dma_async_pattern_all_to_one(uint32_t local_offset, uint32_t remote_offset, size_t transfer_size){
    FlexPosition pos = get_pos(flex_get_cluster_id());
    bare_dma_start_1d(local(local_offset),remote_xyz(0,0,0,remote_offset), transfer_size); //Start iDMA
}

//Pattern Dialog-to-Dialog
// TODO: How to translate this to 3D?
void flex_dma_async_pattern_dialog_to_dialog(uint32_t local_offset, uint32_t remote_offset, size_t transfer_size){
    FlexPosition pos = get_pos(flex_get_cluster_id());
    bare_dma_start_1d(local(local_offset),remote_xyz(pos.z,pos.y,pos.x,remote_offset), transfer_size); //Start iDMA
}
