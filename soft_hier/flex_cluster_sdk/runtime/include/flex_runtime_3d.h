...
#define ARCH_NUM_CLUSTER            (ARCH_NUM_CLUSTER_X*ARCH_NUM_CLUSTER_Y*ARCH_NUM_CLUSTER_Z)
#define ARCH_SYNC_SIZE              (ARCH_SYNC_INTERLEAVE + ARCH_SYNC_SPECIAL_MEM)
#define cluster_index(x,y,z)        ((z)*(ARCH_NUM_CLUSTER_Y*ARCH_NUM_CLUSTER_X)+(y)*ARCH_NUM_CLUSTER_X+(x))
...
FlexPosition zplus_pos(FlexPosition pos) {
    uint32_t new_x = pos.x;
    uint32_t new_y = pos.y;
    uint32_t new_z = (pos.z + 1) % ARCH_NUM_CLUSTER_Z;
    FlexPosition new_pos;
    new_pos.x = new_x;
    new_pos.y = new_y;
    new_pos.z = new_z;
...
FlexPosition zminus_pos(FlexPosition pos) {
    uint32_t new_x = pos.x;
    uint32_t new_y = pos.y;
    uint32_t new_z = (pos.z + ARCH_NUM_CLUSTER_Z - 1) % ARCH_NUM_CLUSTER_Z;
    FlexPosition new_pos;
    new_pos.x = new_x;
    new_pos.y = new_y;
    new_pos.z = new_z;
...
uint32_t flex_get_barrier_num_cluster_z(){
    volatile uint32_t * info_reg      = (volatile uint32_t *) (ARCH_CLUSTER_REG_BASE+32);
    return *info_reg;
}
...
inline void flex_wakeup_clusters(uint8_t row_mask, uint8_t col_mask, uint8_t lay_mask)
{
    // Byte layout (little endian) is, for compatibility: | 0 | col | lay | row |.
    volatile uint32_t * wakeup_reg   = (volatile uint32_t *) (ARCH_CLUSTER_REG_BASE+28);
    uint32_t value = (((uint32_t)col_mask) << 16) | (((uint32_t)lay_mask) << 8) | row_mask;
    *wakeup_reg = value;
}
...
void flex_barrier_xyz_init(){
    FlexPosition        pos          = get_pos(flex_get_cluster_id());
    uint32_t            pos_x_middel = (ARCH_NUM_CLUSTER_X)/2;
    uint32_t            pos_y_middel = (ARCH_NUM_CLUSTER_Y)/2;
    uint32_t            pos_z_middel = (ARCH_NUM_CLUSTER_Z)/2;
    volatile uint32_t * barrier_z    = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos_y_middel,pos_z_middel)*ARCH_SYNC_SIZE)+24);
    volatile uint32_t * cluster_reg  = (volatile uint32_t *) ARCH_CLUSTER_REG_BASE;

    if (flex_is_dm_core()){
        if (flex_get_cluster_id() == 0)
        {
            flex_reset_barrier(barrier_z);
            for (int k = 0; k < ARCH_NUM_CLUSTER_Z; ++k)
            {
                volatile uint32_t * barrier_y = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos_y_middel,k)*ARCH_SYNC_SIZE)+16);
                flex_reset_barrier(barrier_y);
                for (int i = 0; i < ARCH_NUM_CLUSTER_Y; ++i)
                {
                    volatile uint32_t * barrier_x = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,i,k)*ARCH_SYNC_SIZE)+8);
...
void flex_global_barrier_xyz(){
    ...
    if (flex_is_dm_core()){
        ...
        uint32_t            pos_z_middel = (flex_get_barrier_num_cluster_z())/2;
        volatile uint32_t * barrier_x    = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos.y       ,pos.z       )*ARCH_SYNC_SIZE)+8);
        volatile uint32_t * barrier_y    = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos_y_middel,pos.z       )*ARCH_SYNC_SIZE)+16);
        volatile uint32_t * barrier_z    = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos_y_middel,pos_z_middel)*ARCH_SYNC_SIZE)+24);
        volatile uint32_t * cluster_reg  = (volatile uint32_t *) ARCH_CLUSTER_REG_BASE;
        ...
        if ((flex_get_barrier_num_cluster_x() - flex_get_enable_value()) == flex_amo_fetch_add(barrier_x)) {
            ...
            if ((flex_get_barrier_num_cluster_y() - flex_get_enable_value()) == flex_amo_fetch_add(barrier_y)) {
                ...
                // Finally, sync Z
                if ((flex_get_barrier_num_cluster_z() - flex_get_enable_value()) == flex_amo_fetch_add(barrier_z)) {
                    flex_reset_barrier(barrier_z);
                    flex_wakeup_all_clusters();
                }
            }
...
void flex_global_barrier_xyz_polling(){
    ...
    if (flex_is_dm_core()){
        ...
        uint32_t            pos_z_middel = (flex_get_barrier_num_cluster_z())/2;
        volatile uint32_t * barrier_x    = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos.y       ,pos.z       )*ARCH_SYNC_SIZE)+8);
        volatile uint32_t * barrier_ix   = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos.y       ,pos.z       )*ARCH_SYNC_SIZE)+12);
        volatile uint32_t * barrier_y    = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos_y_middel,pos.z       )*ARCH_SYNC_SIZE)+16);
        volatile uint32_t * barrier_iy   = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos_y_middel,pos.z       )*ARCH_SYNC_SIZE)+20);
        volatile uint32_t * barrier_z    = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos_y_middel,pos_z_middel)*ARCH_SYNC_SIZE)+24);
        volatile uint32_t * barrier_iz   = (volatile uint32_t *) (ARCH_SYNC_BASE+(cluster_index(pos_x_middel,pos_y_middel,pos_z_middel)*ARCH_SYNC_SIZE)+32);

        // Remember previous iteration
        uint32_t prev_barrier_iter_x     = *barrier_ix;
        uint32_t prev_barrier_iter_y     = *barrier_iy;
        uint32_t prev_barrier_iter_z     = *barrier_iz;
        ...
        if ((flex_get_barrier_num_cluster_x() - flex_get_enable_value()) == flex_amo_fetch_add(barrier_x)) {
            ...
            if ((flex_get_barrier_num_cluster_y() - flex_get_enable_value()) == flex_amo_fetch_add(barrier_y)) {
                ...
                if ((flex_get_barrier_num_cluster_z() - flex_get_enable_value()) == flex_amo_fetch_add(barrier_z)) {
                    flex_reset_barrier(barrier_z);
                    flex_amo_fetch_add(barrier_iz);
                } else {
                    while((*barrier_iz) == prev_barrier_iter_z);
                }
