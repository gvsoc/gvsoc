...
PLPTEST_CMD = plptest $(PLPTEST_TARGET_FLAGS) --max-timeout 60 run table junit
...
test.checkout.pulp-sdk:
	...
	cd "tests/pulp-sdk" && \
	git fetch --all && \
	git checkout 7b90a7bc0a50dd0b28fa751b0ea616966069bbde
...
test.checkout.snitch:
	@if [ ! -d "tests/snitch" ]; then \
		git clone "git@github.com:pulp-platform/snitch_cluster.git" "tests/snitch"; \
	fi
	cd "tests/snitch" && \
	git fetch --all && \
	git checkout c4861daf751993f5ebccee507e2bfa1fb2675cac && \
...
test.build.snitch: test.checkout.snitch
	...
	export PATH=$(LLVM_BINROOT):$(CURDIR)/tests/snitch/install/bin:$(PATH) && cd tests/snitch/target/snitch_cluster && make DEBUG=ON OPENOCD_SEMIHOSTING=ON sw
